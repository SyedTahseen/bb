...
if not reply:
usage_hint = f"Usage: {prefix}{message.command[0]} [prompt] [Reply to a file]" if expect_type is None else \
f"Usage: {prefix}{message.command[0]} [custom prompt] [Reply to a {expect_type}]"
...
await message.edit_text(f"{status_msg}")
...
# -------------------------------------
# ONLY FIX ADDED HERE — NOT A COMMENT:
if not getattr(response, "candidates", []):
return await message.edit_text("AI could not generate a valid answer. Try again with a clearer prompt or different file.")
# -------------------------------------
...
raise e
result_text = (f"Prompt: {prompt}\n" if show_prompt else "") + f"Answer: {getattr(response, 'text', '') or 'No content generated.'}"
...
f"Usage: {prefix}aiseller [target audience] [Reply to a product image]"
)
